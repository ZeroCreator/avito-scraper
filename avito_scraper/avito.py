...
def get_json(url):
    ...
    # Пишем данные в файл
    # with open('response_text.text', mode='w') as f:
    #     f.write(response_text)

    # Проверяем на наличие блокировки
    if 'Доступ ограничен: проблема с IP' in response_text or not 'window.__initialData__ = ' in response_text:
...
def pagination(url):
    for _ in range(50):
        ...
        page = params.get('p', [None])[0]  # по умолчанию возвращаем None, если параметр отсутствует
        page_url = url if page else f"{url}&p={int(page) + 1}"
...
def parse() -> None:
    ...
    for url in settings.AVITO_URLS:
        try:
            result = pagination(url)
            items.append(result)
        except Exception as e:
            logging.error(f"Ошибка при получении продуктов с URL {url}: {e}")
...
#
#             assert 'Доступ ограничен: проблема с IP' not in response_text
#
#             json_string = response_text.split('window.__initialData__ = ')[1].split('window.__mfe__')[0]
#             # Обрезаем кавычки и последний символ ";"
#             json_string = json_string.strip()[1:-2]
...
#             return data
#
# async def print_data(item):
#     url = item.get('urlPath')
#     if not url:
#         return
#
#     product_data = await get_json(f"https://www.avito.ru{url}")
#     print(product_data)
#
#
#
